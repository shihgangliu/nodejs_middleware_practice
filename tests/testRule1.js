...
describe('Test rule1', function () {
    it('Test wrong file', function () {
        Base.runApp('./testss/request_data/without_headers.json', ['rule1'], function (err, dataObj) {
            Assert.equal(
                "ENOENT: no such file or directory, open './testss/request_data/without_headers.json'",
                err.message
            );
        });
    })
    ...
    it('Test rules must be array', function () {
        Base.runApp('./tests/request_data/without_headers.json', 'rule1', function (err, dataObj) {
